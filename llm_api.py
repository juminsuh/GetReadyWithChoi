    ...
    response = client.chat.completions.create(
        ...
        {   
            ...
            "content": [
                ...
                # 8
                {"type": "text", "text": f"네 번째 착장 이미지"},
                {"type": "image_url", "image_url": {"url": src_url}},
                {"type": "text", "text": f"위 세 가지 예시를 참고해 네 번째 착장 이미지에 대한 상의, 하의, 가방, 신발, TPO를 묘사해줘. 반환은 JSON 텍스트로만, 스키마의 키 (top, bottom, bag, shoes, TPO)만 사용해. items 배열에 label은 {label} 하나만 넣어."}
...
def main():
    ...
    save_path = os.path.join("json", filename)
    
    with open("json/snap_dedup.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    for i, item in enumerate(data):
        ...
        json_response = gpt(response_format=response_format, label=label, tpo=tpo, src_url=src_url)
